...
def clean_flight_data(flights):
    ...
    # drop route data since it is covered by stops. 
    flights.drop(["Route"], axis = 1, inplace = True)

    # drop additional info as it is mostly redundant and not well defined
    ...
    # drop duration as it is dependent on stops, source, and destination
    flights.drop(["Duration"], axis = 1, inplace = True)
